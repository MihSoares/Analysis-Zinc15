...
df$ExactMolWt=as.integer(df$ExactMolWt)
df$MolLogp=as.integer(df$MolLogp)
df$TPSA=as.integer(df$TPSA)
...
ggplot(MMM) +
  aes(x = Descriptors, y = Mean, colour = Descriptors) +
  geom_col(fill = "#112446") +
  scale_color_hue(direction = 1) +
  labs(title = "Histogram", subtitle = "Mean Valors") +
  theme_bw()

# max Valors

ggplot(MMM) +
 aes(x = Descriptors, y = Max, colour = Descriptors) +
 geom_col(fill = "#112446") +
 scale_color_hue(direction = 1) +
 labs(title = "Histogram", subtitle = "Max Valors") +
 theme_bw()

# min Valors

ggplot(MMM) +
  aes(x = Descriptors, y = Min, colour = Descriptors) +
  geom_col(fill = "#112446") +
  scale_color_hue(direction = 1) +
  labs(title = "Histogram", subtitle = "Min Valors") +
  theme_bw()
